...
export default function Hamburger({ open = false }) {
  return (
    <div
      className={`fixed z-20 top-0 right-0 m-4 md:hidden select-none text-center w-11 h-14 transition-all cursor-pointer before:w-full before:h-2 before:bg-miu-500 before:block before:absolute before:rounded after:w-full after:h-2 after:bg-miu-500 after:block after:absolute after:rounded before:origin-left-top after:right-0 after:top-5 after:origin-right-top  ${
        open
    ...
    >
      <span
        className={`text-miu-500 block absolute bottom-[33px] left-[2px] text-sm  ${
          open
