...
export default function Hamburger({ open = false }) {
  return (
    <div
      className={`fixed z-20 top-0 right-0 m-4 md:hidden select-none text-center w-11 h-14 transition-all cursor-pointer before:w-full before:h-2 before:bg-black before:block before:absolute before:rounded after:w-full after:h-2 after:bg-black after:block after:absolute after:rounded before:origin-left-top after:right-0 after:top-5 after:origin-right-top  ${
        open
    ...
    >
      <span
        className={`text-black block absolute bottom-0 -left-[1px] text-sm font-bold ${
          open
